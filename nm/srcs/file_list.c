...
/*   Updated: 2020/07/31 12:21:24 by craffate         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_file					*create_file_node(const char *name)
{
	...
	if (ret)
	{
		if (!(ret->name = ft_strdup(name)))
			ret = NULL;
		if ((ret->fd = open(ret->name, O_RDONLY)) == -1)
			ret = NULL;
		if (fstat(ret->fd, &ret->stat) == -1)
			ret = NULL;
		if ((ret->ptr = (char *)mmap(0, ret->stat.st_size, PROT_READ,
